...
export default {
  ...
  methods: {
    registerAppUser() {
      if (this.username && this.password && this.password_confirmation) {
        if (this.password && this.password_confirmation) {
          ...
          axios
            .post("http://lara-chat-app-starter.test/api/register", userData)
            .then(response => {
              if (response.data) {
    ...
    async createUserOnCometChat(username) {
      let url = "https://api-eu.cometchat.io/v2/users";
      let data = {
        uid: username,
    ...
    sendTokenToServer(token, uid) {
      axios
        .post("http://lara-chat-app-starter.test/api/update/token", {
          token,
          uid
        })
        .then(response => {
